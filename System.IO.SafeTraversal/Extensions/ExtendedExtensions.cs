...
namespace System.IO.SafeTraversal
{
    public partial class SafeTraversal
    {
        ...
        private static IEnumerable<FileInfo> ExTopLevelFilesTraversalWithSafetyCheck(DirectoryInfo path)
        {
            FileInfo[] files = null;
            try
            {
            ...
            if (files != null)
            {
                for (int i = 0; i < files.Length; i++)
                {
                    if (IsSafeFile(files[i].FullName))
                    {
                        yield return files[i];
                    }
                }
        ...
        private static IEnumerable<FileInfo> ExTopLevelFilesTraversalWithSafetyCheck(DirectoryInfo path, Func<FileInfo, bool> filter)
        {
            IEnumerable<FileInfo> files = null;
            try
            {
                files = path.GetFiles()
                    .Where(x =>
                    {
                        ...
                        try
                        {
                            success = filter(x) && IsSafeFile(x.FullName);
                        }
                        catch { success = false; }
                        return success;
                    });
        ...
        private static IEnumerable<string> ExTopLevelFilesTraversalWithSafetyCheck(string path)
        {
            string[] files = null;
            try
            {
        ...
        private static IEnumerable<string> ExTopLevelFilesTraversalWithSafetyCheck(string path, Func<FileInfo, bool> filter)
        {
            IEnumerable<string> files = null;
            try
            {
        ...
        private static IEnumerable<FileInfo> ExTraverseFilesCoreWithSafetyCheck(DirectoryInfo path)
        {
            Queue<DirectoryInfo> directories = new Queue<DirectoryInfo>();
            directories.Enqueue(path);
            while (directories.Count > 0)
        ...
        private static IEnumerable<FileInfo> ExTraverseFilesCoreWithSafetyCheck(DirectoryInfo path, Func<FileInfo, bool> filter)
        {
            Queue<DirectoryInfo> directories = new Queue<DirectoryInfo>();
            directories.Enqueue(path);
            while (directories.Count > 0)
        ...
        private static IEnumerable<string> ExTraverseFilesCoreWithSafetyCheck(string path)
        {
            Queue<string> directories = new Queue<string>();
            directories.Enqueue(path);
            while (directories.Count > 0)
        ...
        private static IEnumerable<string> ExTraverseFilesCoreWithSafetyCheck(string path, Func<FileInfo, bool> filter)
        {
            Queue<string> directories = new Queue<string>();
            directories.Enqueue(path);
            while (directories.Count > 0)
